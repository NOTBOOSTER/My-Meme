...
const Follower = ({userId}) => {
  ...
  const handleFollow = async (username) => {
    await fetch(`/api/user/removeFollow`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const newfollowers = followers.filter(follower => follower.username !== username)
  ...
  if (followers.length === 0 && !loading) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 text-lg">No one follows you 🤡</p>
      </div>
    )
  }
      ...
      <div className="flex flex-row flex-wrap items-center justify-center gap-2">
        {followers.map((follower) => (
          <div key={follower.follower_id
} className="flex flex-row justify-between min-w-80 px-4 py-2 border-2 rounded-md  border-violet-20">
            ...
            <button onClick={() => handleFollow(follower.username)} className="text-gray-700 hover:text-gray-900">
              <IoIosClose size={40}/>
            </button>
          </div>
        ))}
